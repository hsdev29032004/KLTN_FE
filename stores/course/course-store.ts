...
export function useCourseStore() {
  ...
  const purchasedList = useSelector((state: RootState) => state.course?.purchasedList)
  const loadingList = useSelector((state: RootState) => state.course?.loadingList)
  const loadingPurchasedList = useSelector((state: RootState) => state.course?.loadingPurchasedList)
  ...
  return {
    ...
    setPurchasedList: (items: CourseListItem[]) => dispatch(setPurchasedList(items)),
